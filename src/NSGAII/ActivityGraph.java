 ...
 * Edge costs are the time of each activity until its successor.
...
class Vertex implements Serializable
{
    ...
    public Vertex(String nm, Activity act)
    {
        name = nm;
        adj = new LinkedList<Edge>();
        this.act = act;
...
public class ActivityGraph implements Serializable
{
    ...
    /**
     * Add a new edge to the graph using an Activity object.
     */
    public void addEdge(Activity sourceName, Activity destName, double cost)
    {
        Vertex v = getVertex(sourceName);
        Vertex w = getVertex(destName);
        v.adj.add(new Edge(w, cost));
     ...
     * In either case, return the Vertex.
     */
    private Vertex getVertex(Activity a)
    {
        Vertex v = vertexMap.get(a.getID());
        if (v == null)
        {
            v = new Vertex(a.getID(), a);
            vertexMap.put(a.getID(), v);
        }
        return v;
    }

    //Attempt to replace old activity with alternate activity
    //Names should match, but data values will likely be different.
    public void replaceActivity(Activity orig, Activity alt)
    {
        Vertex v = vertexMap.get(orig.getID());
        if (v == null)
        {
        ...
        {
            count++;
            e.cost = Math.abs(v.act.getTime()) * -1;
        }
        vertexMap.put(v.name, v);
    }

    /*
     * Retreive an activity by its id. Returns null when no matching
    ...
    {
        java.util.List<Activity> list = new java.util.ArrayList<Activity>();
        for (Vertex v : vertexMap.values())
        {
            list.add(v.act);
        }
